import shippment from "../model/shippment.js"
...
export const cancelshippment = async (req, res) => {
    try {
        ...
        const shippment = await shippment.findOne({ order_id: order_id });
        if (!shippment) return res.status(403).send('Shippment does not exist');

        if(shippment.shippment_status === 'CREATED') {
            await Shippment.updateOne({ order_id }, { shippment_status: 'CANCELED' });
            return res.status(200).send({ body: 'CANCELED', message: 'Your Shippment canceled successfully' });
        }
        return res.status(200).send({ body: shippment.shippment_status, message: 'cancelletion cant be done' });
    }
    catch (e) {
